...
for rs in sorteados:
    ...
    classifier = SVC(class_weight="balanced", random_state=ramdomState)
    ...
    #Grid Search 
    paramGrid = {
        'estimator__C': np.arange(0.025, 1.025, 0.025).tolist(),
        'estimator__degree': np.arange(3, 26, 1).tolist(),
        'estimator__gamma': ['scale', 'auto'] + np.arange(0.1, 2.1, 0.1).tolist(),
...
dfProcessamento.to_csv('arrayProcessamentoSVM.csv', sep=';', decimal=',')
