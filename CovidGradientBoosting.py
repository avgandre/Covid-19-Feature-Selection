...
#df = pd.read_csv("Datasets/novo_covid_ajustado_com_sintomas1.csv")
df = pd.read_csv("Datasets/novo_covid_ajustado_com_sintomas2.csv")
...
for rs in sorteados:
    ...
    classifier = GradientBoostingClassifier(random_state=ramdomState)
    ...
    paramGrid = {
        ...
        'estimator__learning_rate': np.arange(0.1, 2.1, 0.1).tolist(),
        'estimator__n_estimators': np.arange(3, 101, 1).tolist(),
        'estimator__subsample': np.arange(0.5, 1.1, 0.1).tolist(),
        ...
        'estimator__max_depth': np.arange(1, 6, 1).tolist(),
        }
    ...
    #Armazena a quantidade de features selecionadas
    featureDictionary['QtdeFeatures'] = len(np.where(selector.get_support() == True)[0])
    ...
    #Armazena o resultado dsa curvas
    yPredictionProba = classifier.predict_proba(xTeste)[:, 1]
    featureDictionary['AUCROC'] = roc_auc_score(yTeste, yPredictionProba)    
    featureDictionary['Avg precision recall curve'] = average_precision_score(yTeste, yPredictionProba)
...
dfProcessamento.to_csv('arrayProcessamentoGradienteBoosting.csv', sep=';', decimal=',')
